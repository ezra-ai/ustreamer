...
#define ADD_ADVANCE_HEADERS     \
	...
	if (client->need_initial)
	{
		...
		_A_EVBUFFER_ADD_PRINTF(buf,
							   ...
							   "Set-Cookie: stream_client%s%s=%s/%" PRIx64 "; path=/; SameSite=None; max-age=30; Secure;" RN
							   "Content-Type: multipart/x-mixed-replace;boundary=" BOUNDARY RN RN
							   "--" BOUNDARY RN,
...
static void _http_send_stream(us_server_s *server, bool stream_updated, bool frame_updated)
{
	...
	US_LIST_ITERATE(run->stream_clients, client, { // cppcheck-suppress constStatement
		...
		if (conn != NULL)
		{
			...
			const bool dual_update = (server->drop_same_frames && client->dual_final_frames && stream_updated && client->updated_prev && !frame_updated);
...
static void _http_refresher(int fd, short what, void *v_server)
{
	...
	if (
		frame_updated && server->notify_parent && (ex->notify_last_online != ex->frame->online || ex->notify_last_width != ex->frame->width || ex->notify_last_height != ex->frame->height))
	{
...
static bool _expose_frame(us_server_s *server, const us_frame_s *frame)
{
	...
	if (server->drop_same_frames && frame->online)
	{
		...
		if (
			(need_drop = (ex->dropped < server->drop_same_frames)) && (maybe_same = us_frame_compare(ex->frame, frame)))
		{
